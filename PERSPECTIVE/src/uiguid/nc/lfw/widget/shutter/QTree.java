...
public class QTree extends BaseComposite {
	...
	// item的图标文字布�?��图标大小数组，按照层次关系保存，顶层的由itemLayouts[0]指定，等�?
	...
	// 按键处理的映射表，key是按键�?，value是动作的id
	private Map<Integer, Integer> keyActionMap;	
	// 用户自定义按键的映射表，key是按键�?，vlaue是Runnable对象
	...
	protected void handleKeyDown(Event event) {
		...
		if (action == SWT.NULL) {		    
		    // 如果没有事件和这个按键绑定，则要�?��按键的内容，如果不包含一�?
		    // 特殊的按键，则当作普通的输入处理，否则忽略掉	
			// 对于普�?的输入，tree的动作是快�?查找
	...
	/**
	 * 查找文本是以character�?��（或者拼音以这个�?��）的item，显示这个item
	 * 
	 * @param character
	 * 		首字符或拼音首字�?
	 */
	...
	/**
	 * Item的expanded状�?改变之后，重画这个item之下的所有item，并�?
	 * 要更新滚动条
	 * 
	 * @param item
	 * 		状�?发生变化的item
	...
	/**
	 * 如果存在这个item，跳到这个item的位�?
	 * 
	 * @param item
	 * 		要显示的Item
	 * @param expand
	 * 		true表示如果这个item不可见，则展�?��父item
	 */
    ...
    /**
     * 得到�?��中文字符串的第一个汉字的声母，如果参数的第一个字母是英文，则返回英文
     * 
     * @param s
     * 		字符�?
     * @return
