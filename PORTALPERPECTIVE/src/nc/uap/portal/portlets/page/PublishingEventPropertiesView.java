...
import nc.uap.portal.portlets.action.AddPublishingEventPropAction;
import nc.uap.portal.portlets.action.DelPublishingEventPropAction;
...
/**
 * PublishingEvent 模型视图
 * 
 * @author dingrf
 *
 */
public class PublishingEventPropertiesView extends Composite {
	...
	public PublishingEventPropertiesView(Composite parent, int style) {
		super(parent, style);
	...
	public void createPartControl(Composite parent) {
		...
		createColumn(tree, PublishingEventCellModifier.colNames[0], 200, SWT.LEFT, 0);
		PublishingEventProvider provider = new PublishingEventProvider();
		tv.setLabelProvider(provider);
		tv.setContentProvider(provider);
		tv.setColumnProperties(PublishingEventCellModifier.colNames);
		//每个Cell的编辑Editor，todo
		CellEditor[] cellEditors = new CellEditor[PublishingEventCellModifier.colNames.length];
		cellEditors[0] = new ObjectComboCellEditor(tree, getPorcessingEvents());
		tv.setCellEditors(cellEditors);
		tv.setCellModifier(new PublishingEventCellModifier(this));
		Action addProp = new AddPublishingEventPropAction(this);
		Action delProp = new DelPublishingEventPropAction(this); 
